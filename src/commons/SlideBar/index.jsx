...
function SlideBar(props) {
  ...
  const {items} = useSelector(state => state.DatastoresReducer);
  ...
  const handleRender = (type, item) => {
    localStorage.setItem('datastores_id',item.d_id);
    props.setRenderItem(item.display_id);
    if (type === 3) {
    }
    else if (type === 4) {
